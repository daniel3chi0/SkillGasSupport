...
#include "AbilitySystem/SkillAttributeSet.h"
...
#include "Component/Werewolf_PawnExtensionComponent.h"
#include "Kismet/GameplayStatics.h"
#include "Manager/SkillManager.h"
#include "western_map/WerewolfGameplayTags.h"


UWerewolf_Ability::UWerewolf_Ability()
{
}

UWerewolf_AnimConfig* UWerewolf_Ability::GetCharaterAnimConfig()
{
	AActor* Avatar = GetAvatarActorFromActorInfo();
	if (Avatar)
	{
		if(UWerewolf_PawnExtensionComponent* PEC = Avatar->GetComponentByClass<UWerewolf_PawnExtensionComponent>())
		{
			if(const UWerewolf_CharacterConfig* CharacterConfig = PEC->GetCharacterData<UWerewolf_CharacterConfig>())
			{
...
void UWerewolf_Ability::SetChooseNum_Implementation(FScalableFloat InNum)
{
	//仅仅修改了服务器的CDO
...
bool UWerewolf_Ability::SetChooseNum_Validate(FScalableFloat InNum)
{
	return true;
}

const FGameplayTagContainer* UWerewolf_Ability::GetCooldownTags() const
{
...
void UWerewolf_Ability::ApplyCooldown(const FGameplayAbilitySpecHandle Handle,
...
FGameplayAbilityTargetDataHandle UWerewolf_Ability::MakeGameplayAbilityTargetDataHandleFromActorArray(const TArray<AActor*> TargetActors)
{
...
FGameplayAbilityTargetDataHandle UWerewolf_Ability::MakeGameplayAbilityTargetDataHandleFromHitResults(const TArray<FHitResult> HitResults)
{
...
FGameplayAbilityTargetDataHandle UWerewolf_Ability::MakeTargetDataFromTargetType(TSubclassOf<UTargetType> TargetType, const FGameplayEventData& EventData)
{
	// 根据目标类型获取目标
...
FGameplayEffectContainerSpec UWerewolf_Ability::MakeEffectContainerSpecFromContainerMap(FGameplayTag ContainerTag, const FGameplayEventData& EventData,
...
TArray<FActiveGameplayEffectHandle> UWerewolf_Ability::ApplyEffectContainerSpecFromContainerMap(FGameplayTag ContainerTag, const FGameplayEventData& EventData, int32 OverrideGameplayLevel)
{
	FGameplayEffectContainer* FoundContainer = EffectContainerMap.Find(ContainerTag);
...
FGameplayTag UWerewolf_Ability::GetFirstAbilityTriggersEventTag()
{
...
UGameplayEffect* UWerewolf_Ability::GetCostGameplayEffect() const
{
...
bool UWerewolf_Ability::CanActivateAbilityReturnMsg(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,
...
bool UWerewolf_Ability::CheckCost(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,
...
void UWerewolf_Ability::ApplyCost(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,
	const FGameplayAbilityActivationInfo ActivationInfo) const
{
	...
	auto DetermineIfAbilityHitTarget = [&]()
	{
		...
		{
			if (UWerewolf_AbilitySystemComponent* ASC = Cast<UWerewolf_AbilitySystemComponent>(ActorInfo->AbilitySystemComponent.Get()))
			{
				FGameplayAbilityTargetDataHandle TargetData;
...
bool UWerewolf_Ability::DoesAbilitySatisfyTagRequirements(const UAbilitySystemComponent& AbilitySystemComponent,
                                                          ...
                                                          FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	const UWerewolf_AbilitySystemComponent* ASC = Cast<UWerewolf_AbilitySystemComponent>(&AbilitySystemComponent);
	...
	if (AllBlockedTags.Num() || AllRequiredTags.Num())
	{
		...
		if (AbilitySystemComponentTags.HasAny(AllBlockedTags))
		{
			...
			{
				OptionalRelevantTags->AddTag(FWerewolfGameplayTags::Ability_ActivateFail_IsDead);
			}
...
bool UWerewolf_Ability::DoesAbilitySatisfyTagRequirementsWithMsg(const UAbilitySystemComponent& AbilitySystemComponent, const FGameplayTagContainer* SourceTags,
...
void UWerewolf_Ability::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo,
