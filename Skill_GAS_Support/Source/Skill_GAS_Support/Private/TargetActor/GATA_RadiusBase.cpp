...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GATA_RadiusBase)
...
AGATA_RadiusBase::AGATA_RadiusBase(const FObjectInitializer& ObjectInitializer)
...
void AGATA_RadiusBase::StartTargeting(UGameplayAbility* InAbility)
{
	Super::StartTargeting(InAbility);
	SourceActor = InAbility->GetCurrentActorInfo()->AvatarActor.Get();
}

void AGATA_RadiusBase::ConfirmTargetingAndContinue()
{
...
FGameplayAbilityTargetDataHandle AGATA_RadiusBase::MakeTargetData(const TArray<TWeakObjectPtr<AActor>>& Actors, const FVector& Origin) const
{
	if (OwningAbility)
	{
...
TArray<TWeakObjectPtr<AActor> >	AGATA_RadiusBase::PerformOverlap(const FVector& Origin)
{
	...
	SourceActor->GetWorld()->OverlapMultiByObjectType(Overlaps, Origin, FQuat::Identity, FCollisionObjectQueryParams(ObjectQueryCollisionChannel), FCollisionShape::MakeSphere(Radius), Params);
